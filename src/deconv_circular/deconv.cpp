...
//		SetupWavHead(fp2, formatChunkBuf, dataSize);
		...
		for (int ch = 0; ch < num_ch; ch++) {
			...
			out_data[ch] = new double[wavSamples];
		}
		...
		while (aCurPosition < wavSamples) {
			...
			int aReadDataSize = (aRemainSize >= sFFTLength) ? sFFTLength : aRemainSize;
			bool aIsFullfilled = (aRemainSize >= sFFTLength) ? true : false;
			...
			for (int ch = 0; ch < num_ch; ch++) {
				...
				for (int i = 0; i < sFFTLength; i++) {
					double aData = REAL(out_fft_data[ch], i) / sFFTLength;
		...
		MaintainWavHeader(FP_OutWaveFile, wavSamples * formatChunkBuf.blockSize, dataStartPos);
	}
}
