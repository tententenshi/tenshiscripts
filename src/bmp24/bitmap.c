...
Image *Read_Bmp(char *filename)
{
	...
	for(i=0; i<height; i++){
		...
		for(j=0; j<width; j++){
			img->rgb[(height-i-1)*width + j].b = bmp_line_data[j*3];
			img->rgb[(height-i-1)*width + j].g = bmp_line_data[j*3 + 1];
			img->rgb[(height-i-1)*width + j].r = bmp_line_data[j*3 + 2];
		}
	}
...
int Write_Bmp(char *filename, Image *img)
{
	...
	for(i=0; i<img->height; i++){
		for(j=0; j<img->width; j++){
			bmp_line_data[j*3]			=	img->rgb[(img->height - i - 1)*img->width + j].b;
			bmp_line_data[j*3 + 1]	=	img->rgb[(img->height - i - 1)*img->width + j].g;
			bmp_line_data[j*3 + 2]			=	img->rgb[(img->height - i - 1)*img->width + j].r;
		}
...
Image *Create_Image(int width, int height)
{
	...
	if((img->rgb = (Rgb*)malloc(sizeof(Rgb)*width*height)) == NULL){
		fprintf(stderr, "Allocation error\n");
		free(img);
		return NULL;
	}

	if((img->hsv = (Hsv*)malloc(sizeof(Hsv)*width*height)) == NULL){
		fprintf(stderr, "Allocation error\n");
		free(img->rgb);
		free(img);
		return NULL;
...
void RGBDivision(Image *colorimg, Image *Rimg, Image *Gimg, Image *Bimg)
{
	...
	for(i=0; i<(colorimg->width)*(colorimg->height); i++){
		Rimg->rgb[i].r = Rimg->rgb[i].g = Rimg->rgb[i].b = colorimg->rgb[i].r;
		Gimg->rgb[i].r = Gimg->rgb[i].g = Gimg->rgb[i].b = colorimg->rgb[i].g;
		Bimg->rgb[i].r = Bimg->rgb[i].g = Bimg->rgb[i].b = colorimg->rgb[i].b;
	}
...
void RGBCombination(Image *Rimg, Image *Gimg, Image *Bimg, Image *colorimg)
{
	...
	for(i=0; i<(colorimg->width)*(colorimg->height); i++){
		colorimg->rgb[i].r = Rimg->rgb[i].r;
		colorimg->rgb[i].g = Gimg->rgb[i].g;
		colorimg->rgb[i].b = Bimg->rgb[i].b;
	}
}
...
void Errordiffusion(Image *img)
{
	...
	if((temp = (int*)malloc(sizeof(int)*img->width*img->height)) == NULL){
		fprintf(stderr, "Allocation Error\n");
		exit(1);
	}
	...
	for(i=0; i<img->height; i++){
		for(j=0; j<img->width; j++){
			...
			if(j != img->width-1) temp[i*img->width + j+1] += (5.0/16)*e; 
			if(!j && i !=img->height-1) temp[(i+1)*img->width + j-1] += (3.0/16)*e;
			if(i !=img->height-1) temp[(i+1)*img->width + j] += (5.0/16)*e;
			if(j != img->width-1 && i !=img->height-1) temp[(i+1)*img->width + j+1] += (3.0/16)*e;
		}
	}
